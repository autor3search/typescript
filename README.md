...
| kill the running `eval` process (e.g. `stop -force`) while a benchmark is measuring | nothing is left orphaned, by one of two routes. On Linux and macOS `eval` catches the SIGTERM, signals the detached measurement child's process group and releases the eval lock before exiting (143). Windows has no cross-process signal, so `killGroup` uses `taskkill /T /F`: the tree dies with the process, but no handler runs, so the lock outlives it and is reclaimed by the next `eval`, whose dead-pid check refuses to treat a live lock as stale |
